...
namespace ChimeraTK {
    ...
    public:
        ...
        InterruptWaitingAccessor_impl(
                size_t interruptNum, uint32_t timeout, boost::shared_ptr<UioBackend> dev,
                const RegisterPath& registerPathName, size_t numberOfWords,
        ...
        _timeout(timeout), _backend(dev), _interruptNum(interruptNum) {
        ...
        void doReadTransfer() override {
            if (_timeout > 0) {
                _myQueue.pop_timedwait(_buffer, _timeout);
            } else {
